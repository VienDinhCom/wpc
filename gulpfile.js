...
function logErrors(filePath, messages) {
    ...
    if (messages.length) {
        ...
        console.log(newFilePath.underline);
...
function checkComponentNames() {
    ...
    const errorComponents = fs
        ...
        .filter(
            component =>
                fs
                    .lstatSync(path.join(componentDir, component))
                    .isDirectory() && !component.startsWith(prefix)
        )
...
gulp.task('scripts', function scripts() {
    ...
    return gulp
        ...
        .pipe(
            $.eslint.result(({ filePath, messages }) =>
                logErrors(filePath, messages)
            )
        )
